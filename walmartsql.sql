...
select holiday_flag, sum(weekly_sales) as Total_WeeklySales from walmart group by Holiday_Flag
...
select top 1 date, Weekly_Sales, store, unemployment from Walmart 
order by Unemployment desc

Question 5; what was the total sales made each year?
select year(date) as year, sum(weekly_sales) as total_sales from Walmart
group by year(date)
order by total_sales desc

Question 6; what are the top 3 stores that made the most sales in a week?
select top 3 store, weekly_sales from walmart 
order by weekly_sales desc
...
select store, avg(weekly_sales) as avg_sales from walmart
group by store 
order by store asc